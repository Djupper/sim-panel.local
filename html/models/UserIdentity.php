...
class UserIdentity extends User implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            [['user_name', 'user_password', 'user_email','user_f','user_i','user_o','content'], 'string', 'max' => 255,'message' => Yii::$app->params['messages']['user']['error']['post_long']],
    ...
    {
        if (UserIdentity::findOne(['id' => $user_id, 'status'=>UserIdentity::STATUS_WAIT])) {
            return TRUE;
        }
        return FALSE;
    }
    ...
    {
        if (UserIdentity::findOne(['id' => $user_id, 'status'=>UserIdentity::STATUS_BLOCKED])) {
            return TRUE;
        }
        return FALSE;
    }
    ...
    {
        if (UserIdentity::findOne(['id' => $user_id, 'status'=>UserIdentity::STATUS_ACTIVE])) {
            return TRUE;
        }
        return FALSE;
    }
    ...
    {
        if ($user_id && $model = UserIdentity::findOne($user_id)) {
    ...
    {
        if ($key && !empty($key) && UserIdentity::find()->where(['auth_key_second'=>$key])->exists()) {
            $model = UserIdentity::findOne(['auth_key_second'=>$key]);
            return $model->id;
        }
        return FALSE;
    }
